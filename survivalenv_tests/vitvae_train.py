...
ZDIM = int(sys.argv[6])
vit_num_classes = int(sys.argv[7])
vit_mlp_dim = int(sys.argv[8])
vit_hidden_dim = int(sys.argv[9])
vit_num_heads = int(sys.argv[10])
vit_num_layers = int(sys.argv[11])
d = 20000
...
if PUBLISH:
    import wandb
    wandb.init(project="VAE", notes=f"v{task_id}", name=f"v{task_id}")
    wandb.config = {"task_id": task_id, "batch_size": batch_size, "learning_rate": learning_rate,
                    ...
                    "vit_num_classes": vit_num_classes, "vit_mlp_dim": vit_mlp_dim,
                    "vit_hidden_dim": vit_hidden_dim, "vit_num_heads": vit_num_heads,
...
net = ViTVAE(zdim=ZDIM, vit_num_classes=vit_num_classes, vit_mlp_dim=vit_mlp_dim, vit_hidden_dim=vit_hidden_dim,
             vit_num_heads=vit_num_heads, vit_num_layers=vit_num_layers).to(device)
...
for epoch in range(num_epochs):
    ...
    if min_val_loss is None or val_loss < min_val_loss:
        ...
        min_epoch = epoch
        patience = max_patience
        torch.save(net.state_dict(), f"vitvae_{task_id}_.pth")
...
if min_val_loss is not None:
    torch.save(best_state_dict, f"vitvae_{task_id}_{str(epoch).zfill(3)}_{min_val_loss}.pth")
